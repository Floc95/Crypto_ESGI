function AttackSysteme(){
	...
	self.getFrequenceMap = function(text){
		...
		for (var i = 0; i < text.length; i++) {
			if (!self.searchIn(map, text[i])) {
				...
				self.searchIn(map, text[i]).myValue++;
		};
	...
	self.myIndexOf = function(map, key){
		for (var i = 0; i < map.length; i++) {
			if (map[i].myKey == key)
				return i;
		};
	...
	self.attack = function(content){
		...
		for (var i = 0; i < content.length; i++) {
			var idx = self.myIndexOf(frequenceMap, content[i]);
			var refChar = self.referenceMap[idx].myKey;
			resContent += refChar;

			var charCodeRef = refChar.charCodeAt(0);
			if (charCodeRef > 64 && charCodeRef < 92 && !self.key[charCodeRef - 65])
				self.key[charCodeRef - 65] = content[i];
			else if (refChar == ' ' && !self.key[27])
				self.key[27] = content[i];
			else if (refChar == '.' && !self.key[26])
				self.key[26] = content[i];
			else if (refChar == ',' && !self.key[25])
				self.key[28] = content[i];
		};
