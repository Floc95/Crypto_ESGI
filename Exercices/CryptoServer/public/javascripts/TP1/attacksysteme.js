function AttackSysteme(cryptosys){
	var self = this;
	...
	self.getFrequenceMap = function(text){
		...
		for (var k in map)
			sorted.push([k, map[k].myValue / map.length]);
		// Sort
	...
	self.attack = function(content){
		...
		// Restitution clé
		self.key = new Array(self.alpha.length);
		for (var i = 0; i < self.alpha.length; i++)
			self.key[self.map[self.referenceMap[i][0]]] = frequenceMap[i][0];

		return cryptosys.decode(content, self.key);
	};
